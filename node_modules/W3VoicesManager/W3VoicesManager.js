			...
			// protected
				...
				function _readAndParseJSONData() {
					...
					for (var racename in stData.races) {
						...
						var sRaceUrl = stData.races[racename].url;
						...
						for (var musicname in stData.races[racename].musics) {

							var sMusicUrl = sRaceUrl + stData.races[racename].musics[musicname].url + '_w3.mp3';

							race.musics[musicname] = {
								name : musicname,
								url : sMusicUrl
							};
						...
						for (var warningname in stData.races[racename].warnings) {

							var sWarningUrl = sRaceUrl + stData.races[racename].warnings[warningname].url + '_w3.mp3';

							race.warnings[warningname] = {
								name : warningname,
								url : sWarningUrl
							};
						...
						for (var charactername in stData.races[racename].characters) {
							...
							var sCharacterUrl = sRaceUrl + stData.races[racename].characters[charactername].url;
							...
							for (var actionname in stData.actions) {

								character.actions[actionname] = {
									name : actionname,
									urls : {}
								};

								for (var i = 0, l = stData.actions[actionname].codes.length; i < l; ++i) {

									var sName = stData.actions[actionname].codes[i];

									character.actions[actionname].urls[sName] = {
										name : sName
										sCharacterUrl + sName + '_w3.mp3'
									};
							...
							race.characters[charactername] = character;

						}

						m_stRaces[racename] = race;

						fs.writeFileSync(CST_DEP_Path.join(__dirname, 'data', racename + '.json'), race, 'utf8');
					...
					// character
						...
						// actions
							...
							// formate
								...
								function _actionToFilePath(p_sRace, p_sCharacter, p_sAction) {
									return CST_DEP_Path.join(__dirname, 'sounds', p_sRace, p_sCharacter, p_sAction + '.mp3');
								}

								function _actionToUrl(p_sRace, p_sCharacter, p_sUrlAction) {
						...
						// actions
							...
							this.playAction = function (p_sRace, p_sCharacter, p_sAction, p_fCallback) {
								...
								if (!_isCharacterValid(p_sRace, p_sCharacter)) {
									...
									_play(
										_actionToUrl(p_sRace, p_sCharacter, sRandomedAction),
										_actionToFilePath(p_sRace, p_sCharacter, sRandomedAction),
										p_fCallback
									);
						...
						m_clThis.getRaces().forEach(function (race) {
							...
							m_clThis.getCharacters(race).forEach(function (character) {
								...
								m_clThis.getActions(race, character).forEach(function(action) {
									...
									m_stRaces[race].characters[character].actions[action].codes.codes.forEach(function(code) {
										...
										var sMP3 = _actionToFilePath(race, character, code);

										if (!CST_DEP_FileStream.existsSync(sMP3)) {

											_download(
												_actionToUrl(race, character, code), sMP3, p_fCallback
											);
							...
							m_clThis.getWarnings(race).forEach(function (warning) {
								...
								if (!CST_DEP_FileStream.existsSync(sMP3)) {
									...
									_download(
										m_stRaces[p_sRace].warnings[p_sWarning].url, sMP3, p_fCallback
									);
							...
							m_clThis.getMusics(race).forEach(function (music) {
								...
								if (!CST_DEP_FileStream.existsSync(sMP3)) {
									...
									_download(
										m_stRaces[p_sRace].musics[p_sMusic].url, sMP3, p_fCallback
									);
