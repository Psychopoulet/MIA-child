...
// dépendances
	...
	var
		...
		/*CST_DEP_Player = require('player'))*/
		CST_DEP_Player = function() {

			var m_clThis = this, m_sSound = '';
			...
			this.add = function (p_sFilePath) {
				m_sSound = p_sFilePath;
				return m_clThis;
			};

			this.play = function (p_fCallback) {

				if ('' != m_sSound) {

					var exec = require('child_process').exec;

					exec(m_sSound, function(error, stdout, stderr) {
				...
				// races
					...
					// musics
						...
						// formate
							...
							function _musicToUrl(p_sRace, p_sMusic) {
								return m_stRaces[p_sRace].url + m_stRaces[p_sRace].musics[p_sMusic].url + '_w3.mp3';
							}
					...
					function _play(p_sUrl, p_sFilePath, p_fCallback) {
						...
						if (!CST_DEP_FileStream.existsSync(p_sFilePath)) {
							...
							_download(p_sUrl, p_sFilePath, function () {
								m_clPlayer.stop().add(p_sFilePath).play(p_fCallback);
							});
					...
					// warnings
						...
						this.playWarning = function (p_sRace, p_sWarning, p_fCallback) {
							...
							if (!_isRaceValid(p_sRace)) {
								...
								_play(
									_warningToUrl(p_sRace, p_sWarning),
						...
						m_clThis.getRaces().forEach(function (race) {
							...
							m_clThis.getMusics(race).forEach(function (music) {
								...
								if (!CST_DEP_FileStream.existsSync(sMP3)) {
									...
									_download(
										_musicToUrl(race, music), sMP3, p_fCallback
									);
